...
function gradf = grad_parametric_rosenbrock(x,alpha)
    ...
    gradf(1,1) = 2*(x(1) -1) + 4*alpha*x(1)*(x(1) - x(2));
...
tol = 1e-8;
rho = 0.5; c1 = 1e-1; btmax = 45; tau_kmax = 100; 
...
t1 = tic;
[xbest_MN_a, xseq_MN_a, iter_MN_a, fbest_MN_a, gradfk_norm_MN_a, btseq_MN_a, flag_bcktrck_MN_a, failure_MN_a] = modified_Newton(f,gradf, Hessf, x0_a, 1000, rho, c1, btmax, tol, tau_kmax);
