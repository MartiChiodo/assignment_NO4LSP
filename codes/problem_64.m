...
function hessian_approx = findiff_hess_64(x, h, type_h)
    ...
    for k = 1:n
        ...
        if k == 1
            ...
            stellina = 2*x(1) - x(2);
            H_kk = 1 + (8*hk^2+ 8*hk*rho*cost^2*(sinh(rho*x(k) + 2*rho*hk) - sinh(rho*x(k) + rho*hk)) + rho^2*cost^4*(sinh(rho*x(k) + rho*2*hk)^2 + sinh(rho*x(k))^2 -2*sinh(rho*x(k) + rho*hk)^2) + 2*stellina*rho*cost^2*(sinh(rho*x(k)+ 2*rho*hk) - 2*sinh(rho*x(k)+ rho*hk) + sinh(rho*x(k))))/(2*hk^2);
            i_indices(cont) = k;
            j_indices(cont) = k;
            values(cont) = H_kk;
            cont = cont +1;
        elseif k < n
            ...
            H_kk = 2 + (8*hk^2+ 8*hk*rho*cost^2*(sinh(rho*x(k) + 2*rho*hk) - sinh(rho*x(k) + rho*hk)) + rho^2*cost^4*(sinh(rho*x(k) + rho*2*hk)^2 + sinh(rho*x(k))^2 -2*sinh(rho*x(k) + rho*hk)^2) + 2*stellina*rho*cost^2*(sinh(rho*x(k)+ 2*rho*hk) - 2*sinh(rho*x(k)+ rho*hk) + sinh(rho*x(k))))/(2*hk^2);
            i_indices(cont) = k;
            j_indices(cont) = k;
            values(cont) = H_kk;
            cont = cont +1;
